#!/usr/bin/env node
/**
 * Generic KB build script. Reads a snapshot txt file and outputs index.html.
 * Usage: node build-kb.js <snapshot-file> <output-dir> <title> <slug> <description> <cluster-count> <date>
 */
const fs = require('fs');
...
if (args.length < 7) {
  console.error('Usage: node build-kb.js <snapshot> <outdir> <title> <slug> <description> <cluster-count> <date>');
  process.exit(1);
}

const [snapshotFile, outDir, title, slug, description, clusterCountStr, pubDate] = args;
const clusterCount = parseInt(clusterCountStr);

const src = fs.readFileSync(snapshotFile, 'utf8');
...
for (let i = 0; i < blocks.length; i += 2) {
  ...
  // Body is everything after queries section until the divider
  let bodyLines = [];
  for (let j = bodyStart; j < lines.length; j++) {
    if (lines[j].trim().match(/^-{5,}$/)) break;
    bodyLines.push(lines[j]);
  }
  const body = bodyLines.join('\n').trim();
  if (body) clusters.push({ num, title: titleLine, queries, body });
}

console.log(`Parsed ${clusters.length} clusters from ${snapshotFile}`);

// Auto-generate categories by grouping clusters into groups of ~5-7
const CATS_PER_GROUP = 6;
const categories = [];
const sorted = [...clusters].sort((a, b) => a.num - b.num);

// Simple grouping: every CATS_PER_GROUP clusters
for (let i = 0; i < sorted.length; i += CATS_PER_GROUP) {
  const group = sorted.slice(i, i + CATS_PER_GROUP);
  // Use first cluster's title as category hint
  const catName = `Section ${Math.floor(i / CATS_PER_GROUP) + 1}`;
  categories.push({
    name: catName,
    ids: group.map(c => c.num)
  });
}
...
const canonicalUrl = `https://metricusapp.com/${slug}/`;
...
<head>
  ...
  <script type="application/ld+json">
  {
    ...
    "@type": "TechArticle",
    "headline": "${escTitle} — ${clusters.length} Research Clusters",
...
<body class="min-h-screen bg-dot-grid">
  ...
  <div class="bg-white border-b border-outline-variant">
    <div class="max-w-5xl mx-auto px-6 py-16 text-center">
      <h1 class="font-display text-4xl md:text-5xl text-on-surface font-bold leading-tight mb-4">${escTitle}</h1>
    </div>
  </div>
...
${content}
      ...
      <div class="mt-16 p-6 bg-white border border-outline-variant">
        ...
        <p class="text-sm text-on-surface-variant">Metricus Research (2026). <em>${escTitle}</em>. metricusapp.com/${slug}/</p>
      </div>
    </main>
  </div>
    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div class="flex flex-wrap items-center gap-6">
          ...
          <a href="../geo-knowledgebase/" class="text-xs uppercase tracking-tighter text-outline hover:text-primary transition-colors">GEO Knowledge Base</a>
          <a href="./" class="text-xs uppercase tracking-tighter text-outline hover:text-primary transition-colors">${escTitle}</a>
...
console.log(`Built ${outDir}/index.html with ${clusters.length} clusters`);
